...
function wp_elementy_header_class($class = ''){
    ...
    if(!empty($opt_theme_options['menu_sticky'])) {
        $custom_class[] = 'header-sticky';
    }

    if(!empty($opt_meta_options['menu_sticky'])) {
        $custom_class[] = 'opt-page-sticky';
    }

    if(!empty($opt_meta_options['menu_sticky_fixed'])) {
        $custom_class[] = 'opt-page-fixed';
    }
    ...
    if ( in_array($page_title_layout, array('2', '5')) ) {
        $custom_class[] = 'has-border-bottom';
    }

    echo esc_attr(implode(' ', $custom_class));
}
...
/**
 * run php in default text widget
 * @param  [type] $html [description]
 * @return [type]       [description]
 */
function wp_elementy_php_execute($html) {
...
add_filter('widget_text','wp_elementy_php_execute',100);
